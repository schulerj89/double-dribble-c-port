...
$reportPath = Join-Path $projectRoot 'build\ghidra-reports\gameplay-audio-bank-01.txt'
$headless = Join-Path $GhidraHome 'support\analyzeHeadless.bat'

& (Join-Path $PSScriptRoot 'Prepare-UxRomBanks.ps1') -RomPath $RomPath
New-Item -ItemType Directory -Force -Path $projectDirectory | Out-Null
$env:JAVA_HOME = $JdkHome
& $headless $projectDirectory 'DoubleDribbleGameplayAudioBank1' `
    ...
    -scriptPath $PSScriptRoot -postScript 'ExportGameplayAudioEvidence.java' $reportPath
if ($LASTEXITCODE -ne 0) { throw "Ghidra gameplay-audio analysis exited with code $LASTEXITCODE" }
Write-Host "Ghidra gameplay-audio report: $reportPath"
