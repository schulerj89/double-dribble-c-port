...
public class ExportGameplayLoopEvidence extends GhidraScript {
    private static final long[] BANK0_ANCHORS = {
        ...
        0x8E71L, 0x8E88L, 0x8EBFL, 0x8EE2L, 0x8FE0L, 0x9018L, 0x904DL, 0x9094L,
        0x9102L, 0x914EL, 0x91A6L, 0x9208L, 0x92BDL, 0x9395L, 0x93AEL, 0x9400L, 0x9413L, 0x9418L,
        0x9431L, 0x9490L, 0x9583L, 0x9645L, 0x9698L, 0x96B4L, 0x9707L,
        0x98B5L, 0xA014L, 0xA0B2L, 0xA0C3L, 0xA0D9L, 0xA0DAL, 0xA129L, 0xA1CCL,
        0xA21FL, 0xA29DL, 0xA347L, 0xA3E2L, 0xA44BL, 0xA482L, 0xA504L, 0xA556L, 0xA557L,
        0xA61FL, 0xA68AL, 0xA6C3L, 0xA712L, 0xA765L, 0xA780L, 0xA7EAL,
        0xA85AL, 0x993AL, 0x9B42L, 0x9E70L, 0x9FA3L, 0xAA20L, 0xAA75L, 0xAAEEL,
        0xAC83L, 0xACABL, 0xACB6L, 0xACD6L,
        0xAD41L, 0xAD58L, 0xAD6DL, 0xADF2L, 0xAE0CL, 0xAE25L, 0xAEDEL,
        0xAF46L, 0xAF72L, 0xAFDDL, 0xB017L, 0xB138L, 0xB167L, 0xB189L, 0xB377L, 0xB435L,
        0xB501L,
        0xB473L, 0xB51DL, 0x9ABDL
    };
    private static final long[] FIXED_ANCHORS = {
        0xC141L, 0xC41CL, 0xC477L, 0xC694L, 0xC6ADL, 0xCC94L, 0xCD24L, 0xCD5CL, 0xCD64L,
    ...
    public void run() throws Exception {
        ...
        try (PrintWriter report = new PrintWriter(output, "UTF-8")) {
            ...
            if (!"fixed".equals(getScriptArgs()[0])) {
                ...
                printDispatchTable(report, "user player", 0xA01CL, 0x00, 16);
                printDispatchTable(report, "ball", 0xAC91L, 0x00, 13);
                printByteTable(report, "CPU avoidance direction", 0x8BC8L, 32);
                printByteTable(report, "inbound target offsets", 0x9761L, 16);
            }
            for (long value : anchors) {
                Address address = toAddr(value);
