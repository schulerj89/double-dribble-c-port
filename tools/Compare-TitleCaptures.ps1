...
if ($OriginalPath -eq '') { $OriginalPath = Join-Path $projectRoot 'captures\original\frame-0030.png' }
if ($NativePath -eq '') { $NativePath = Join-Path $projectRoot 'captures\native\title.png' }
$diffRoot = Join-Path $projectRoot 'captures\diff'
$diffPath = Join-Path $diffRoot 'title-diff.png'
...
try {
    ...
    for ($y = 0; $y -lt $original.Height; $y++) {
        for ($x = 0; $x -lt $original.Width; $x++) {
            ...
            # This FCEUX installation captures configured scanlines 8..231.
