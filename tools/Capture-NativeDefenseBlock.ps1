...
foreach ($phase in @('contact', 'landing')) {
    ...
    & $CliPath --render-gameplay-block $AssetPackPath $phase $bmpPath
    if ($LASTEXITCODE -ne 0) { throw "Native defensive-block rendering failed at $phase." }
    ...
    Write-Host "Native defensive-block frame: $pngPath"
}
